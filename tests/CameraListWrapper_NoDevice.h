...
class CameraListWrapper_NoDevice : public CxxTest::TestSuite {
  public:
    ...
    void testConstructionAndReAssignmentWithSort() {
        ...
        TS_ASSERT_THROWS(cameraList.getName(10),
                         gphoto2pp::exceptions::gphoto2_exception); // out of bounds
    }
    ...
    void testDuplicatesWithFindByName() {
        ...
        cameraList.append("First Model", "Some Port1");
        cameraList.append("First Model",
                          "Some Port1"); // Should I make the append method throw an error if the
                                         // model aleady exists on the port? Best to ask Marcus
    ...
    void testCopyAndAssignmentOperators() {
...

        {
            ...
            TS_ASSERT_THROWS(
                cameraList.count(),
                gphoto2pp::exceptions::gphoto2_exception); // The old value is no longer valid
