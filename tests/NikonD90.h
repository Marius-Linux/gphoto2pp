...
public:
    ...
    void testRetrievalWrongWidgetType() {
        ...
        TS_ASSERT_THROWS(rootWidget.getChildByName<gphoto2pp::ToggleWidget>("iso"),
                         gphoto2pp::exceptions::InvalidWidgetType);
    }
    ...
    void testISO_RadioWidget() {
        // First lets get the current iso, and save the current setting, so we can restore the
        // camera to the settings we found it at.
        auto isoWidget = _camera.getConfig().getChildByName<gphoto2pp::RadioWidget>("iso");
    ...
    void testAutoFocus_ToggleWidget() {
        ...
        if (focusMode2.getValue() == "MF (fixed)" || focusMode2.getValue() == "MF (selection)") {
            ...
            auto afsWidget =
                _camera.getConfig().getChildByName<gphoto2pp::ToggleWidget>("autofocusdrive");

            // Toggling 1 makes an AF lens auto focus. Not sure what happens if it's a manual focus
            // lens, or the m/f switch is triggered. Could make test more robust by putting try{}
            // catch{} around it, and if it fails, we read some value to determine if the lens is
            // set into M/F mode
    ...
    void testFlashExposure_RangeWidget() {
        // First lets get the current flashexposurecompensation, and save the current setting, so we
        // can restore the camera to the settings we found it at.
        auto exposureWidget =
            _camera.getConfig().getChildByName<gphoto2pp::RangeWidget>("flashexposurecompensation");
    ...
    void testImageQuality_MenuWidget() {
        // 5004 Corresponds to Nikon D90 Image Type menu, 0=JPEG basic, 1=JPEG normal, 2=JPEG fine,
        // 4=RAW, 5=RAW+basic, 6=RAW+normal, 7=RAW+fine 5003 Corresponds to Nikon D90 Image Size
        // menu (only relevant for 5004 options with Jpeg in it)

        // First lets get the current image quality and image size values and store them,so we can
        // restore the camera after testing is complete.

        // Because we are making two calls, we will store the rootWidget in a pointer to avoid
        // gphoto from having to construct the tree twice.
        auto rootWidget = _camera.getConfig();
        ...
        // If it's raw, we need to set the camera's type to Jpeg Basic first before we can set the
        // size.
    ...
    void testBatteryLevel_TextWidget() {
        auto batteryLevelWidget =
            _camera.getConfig().getChildByName<gphoto2pp::TextWidget>("batterylevel");
    ...
    void testActions_SectionWidget() {
        auto actionWidget =
            _camera.getConfig().getChildByName<gphoto2pp::NonValueWidget>("actions");

        TS_ASSERT_LESS_THAN_EQUALS(
            3, actionWidget.countChildren()); // (because 2.4.14 has 3 children, and 2.5 or greater
                                              // has 4 children, actually 2.5.3 has 5 children)
    }

    // This is the test that sets the capturetarget widget to SD instead of internal memory
    void testCaptureToSD() {
        auto radioWidget =
            _camera.getConfig().getChildByName<gphoto2pp::RadioWidget>("capturetarget");
    ...
    void testgetAllByWidgetType() {
        auto widgets = gphoto2pp::helper::getAllWidgetsNamesOfType(
            _camera.getConfig(), gphoto2pp::CameraWidgetTypeWrapper::Section);
    ...
    void testgetAllByWidgetTypeFullname() {
        auto widgets = gphoto2pp::helper::getAllWidgetsNamesOfType(
            _camera.getConfig(), gphoto2pp::CameraWidgetTypeWrapper::Section, true);
