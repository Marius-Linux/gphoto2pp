...
public:
    ...
    void testSummary() { TS_ASSERT(!_camera.getSummary().empty()); }
    ...
    void testMoveConstructor() {
        ...
        _camera =
            std::move(moveconstructor); // now assign it back, ironically using move assignment
    ...
    void testSDCardAccessingAndDeletingFiles() {
        // The previous test should have taken a picture and put the temporary image in the root
        // folder
        // "/"
        ...
        TS_ASSERT_THROWS(_camera.fileDelete(_captureFilePath.Folder, _captureFilePath.Name);
    ...
    void testSDCardAccessingFolders() {
        ...
        // Makes sure we have at least one sub folder, which we should at the root of the camera
        // filesystem
