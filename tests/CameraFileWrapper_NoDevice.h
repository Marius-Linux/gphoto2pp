...
class CameraFileWrapper_NoDevice : public CxxTest::TestSuite {
  ...
  public:
    ...
    void testInitializeFile() {
        // Now we don't necessarily need to get a legitimate file (could read one from disk or use a
        // dummy one), but I prefer to get a file from the camera to do all the tests.
        //~ gphoto2pp::CameraWrapper _camera;
        //~ auto filePath = _camera.capture(gphoto2pp::CameraCaptureTypeWrapper::Image);
        //~ _file = _camera.fileGet(filePath.Folder, filePath.Name,
        ...
        std::ifstream in("unit_test_sample_input.jpg",
                         std::ios::in | std::ios::ate | std::ios::binary);
        if (in) {
            //~ std::vector<char> fileContents((std::istreambuf_iterator<char>(in)),
            // std::istreambuf_iterator<char>()); // This is a shorter and cleaner way, but will
            // resize the vector as it's made.
            std::vector<char> fileContents;
            fileContents.reserve(in.tellg());
            in.seekg(0, std::ios::beg);
            fileContents.assign(std::istreambuf_iterator<char>(in),
    ...
    void testSetFileName() { _file.setFileName(fullFileName); }
    ...
    void testMoveConstructor() {
        ...
        TS_ASSERT_THROWS(
            _file.getFileName(),
            gphoto2pp::exceptions::gphoto2_exception); // The old value is no longer valid again

        _file = std::move(moveconstructor); // now copy it back, ironically using move assignment
                                            // which we previously checked that it was working
    ...
    void testMimeFunctions() {
        _file.detectMimeType(); // By default it will be unknown/unknown, this will now change it to
                                // hopefully image/png
        auto oldMimeType =
            _file.getMimeType(); // now with our sample file this should be image/jpeg
        ...
        // Originally I used setMimeType, however this is a perfect opportunity to test
        // detectMimeType()
    ...
    void testTimeAccessors() {
        // I don't have any better way to test the time set and get for now. I just read, write it
        // back with 5 seconds later, and then read and diff the two.
