 ...
 * This capture type might not be supported by all cameras (requires a live view/mirror lockup mode
 * for continuous captures) \param[in]	cameraWrapper	instance which will be used to issue the
 * capture_preview command \param[in]	outputFilename	will be used when saving the captured image
 * to disk \throw GPhoto2pp::exceptions::gphoto2_exception
 */
 ...
 * This capture type might not be supported by all cameras (requires a live view/mirror lockup mode
 * for continuous captures) \param[in]	cameraWrapper	instance which will be used to issue the
 * capture_preview command \param[out]	outputStream	will contain the file data in the ostream
 * memory \throw GPhoto2pp::exceptions::gphoto2_exception
 */
 ...
 * \param[in]	autoDeleteImageFromSrc	will remove the file from temporary memory after it's
 * contents have been copied into the cameraFile. The capt0000 is never cleaned up automatically
 * until the camera exists. Most cases will want this set to true, especially long running programs.
 * \param[in]	captureType	indicates to the camera what file we want (Image, Sound, Movie)
 * \param[in]	fileType	indicates the type of file to capture from the camera (Raw, Normal,
 * exif, etc...). Most modern cameras can leave this as default. \throw
 * GPhoto2pp::exceptions::gphoto2_exception
 */
void capture(CameraWrapper &cameraWrapper, CameraFileWrapper &cameraFile,
 ...
 * GPhoto2pp::exceptions::gphoto2_exception
 */
void capture(CameraWrapper &cameraWrapper, std::string const &outputFilename,
             bool autoDeleteFileFromSrc = false,
             CameraCaptureTypeWrapper const &captureType = CameraCaptureTypeWrapper::Image,
             CameraFileTypeWrapper const &fileType = CameraFileTypeWrapper::Normal);
 ...
 * GPhoto2pp::exceptions::gphoto2_exception
 */
void capture(CameraWrapper &cameraWrapper, std::ostream &outputStream,
             bool autoDeleteFileFromSrc = false,
             CameraCaptureTypeWrapper const &captureType = CameraCaptureTypeWrapper::Image,
             CameraFileTypeWrapper const &fileType = CameraFileTypeWrapper::Normal);
