...
class CameraWrapper {
  public:
    ...
    // This is not copyable. the t_Camera object is sort of a singleton. We don't want multiple
    // commands being issued to one t_Camera resource.
     ...
     * This capture type might not be supported by all cameras (requires a live view/mirror lockup
     * mode for continuous captures). The image does not persist on the camera. \return the image
     * captured \note Direct wrapper for <tt>gp_camera_capture_preview(...)</tt> \throw
    ...
    /**
     * \brief Queries all the properties/abilities on the camera and compiles them in a Widget N-way
     * tree hierarchy. \return the Root widget (which will always be of type Window Widget) \note
     * Direct wrapper for <tt>gp_camera_get_config(...)</tt> \throw
     * GPhoto2pp::exceptions::gphoto2_exception
     */
     ...
     * It's important to note, that if camera settings change (manually by physical interaction),
     * and then you call this method with the old settings, it will change the camera back to the
     * old settings. It's best practice to query the camera, change the settings, and then
     * immediately set
     * the config again. \param[in]	cameraWidget	to traverse and write all settings to the
     * camera \return the Root widget (which will always be of type Window Widget) \note Direct
     * wrapper for <tt>gp_camera_set_config(...)</tt> \throw
     * GPhoto2pp::exceptions::gphoto2_exception
     */
     ...
     * \throw GPhoto2pp::exceptions::gphoto2_exception
     */
    void folderPutFile(std::string const &folder, std::string const &fileName,
                       CameraFileTypeWrapper const &fileType, CameraFileWrapper cameraFile);
     ...
     * \throw GPhoto2pp::exceptions::gphoto2_exception
     */
    CameraFileWrapper fileGet(std::string const &folder, std::string const &fileName,
     ...
     * This method should be used to setup all the callbacks necessary before calling
     * startListeningForEvents. Provide your callback method with lambdas or static/instance methods
     * (use std::bind). \par Event Types:
     * - FileAdded
     * - FolderAdded
     * - CaptureComplete
     * - Timeout
     ...
     * \throw GPhoto2pp::exceptions::gphoto2_exception
     */
    observer::Registration subscribeToCameraEvent(
     ...
     * You must subscribe to at least one event type and then perform some action on the camera to
     * see this in action. \return true if started listening for events, false if already listening
     * to events
     */
     ...
     * If there is no thread, it leaves the method immediately, if there is a thread running, it is
     * a blocking call which will not return until the thread exits.
     */
  ...
  private:
    ...
    /**
     * \brief Initializes a specific camera. You can obtain the model and port using the
     * gphoto2pp::helper::autoDetectAll \param[in]	model	of the camera to connect to
    ...
    observer::SubjectEvent<CameraEventTypeWrapper,
                           void(const CameraFilePathWrapper &, const std::string &)>
        m_cameraEvents;
