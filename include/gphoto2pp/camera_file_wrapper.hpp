...
public:
    ...
    /**
     * \note this copy constructor does not actually copy the image, but rather makes a copy of this
     * wrapper around the gphoto2 struct CameraFile.
     */
    CameraFileWrapper(CameraFileWrapper const &other);

    /**
     * \note this copy assignments does not actually copy the image, but rather makes a copy of this
     * wrapper around the gphoto2 struct CameraFile.
     */
     ...
     * This structure is acceptable for transportation of the camera file, as it has a size() method
     * and c_str() to retrieve the relevant binary picture data. \return the picture \note Direct
     * wrapper for <tt>gp_file_get_data_and_size(...)</tt> \throw
     ...
     * If the MIME type is still unknown/unknown after this, then gphoto2 does not recognize the
     * file type. \note Direct wrapper for <tt>gp_file_detect_mime_type(...)</tt> \throw
     * GPhoto2pp::exceptions::gphoto2_exception
     */
     ...
     * \param[in]	baseName	for this file, which will appear after the file type and
     * before the mime type \param[in]	type		of file that was captured (Image, Movie,
     * Sound) \note Direct wrapper for <tt>gp_file_get_name_by_type(...)</tt> \throw
     * GPhoto2pp::exceptions::gphoto2_exception
     */
    std::string getNameByType(std::string const &baseName, CameraFileTypeWrapper const &type)
        const; // This command is only supported by GPhoto >= 2.5
