 ...
 * A class representing gphoto2 widgets which are not leaf nodes (have children), which is a Window
 * or Section widget
 */
...
protected:
    ...
    /**
     * \brief Gets the child widget by the name passed in and returns the widget type that the user
     * expects. \tparam a type that inherits from CameraWidgetWrapper \param[in]	name	of
     * the child widget to get \return the widget \note Direct wrapper for
     * <tt>gp_widget_get_child_by_name(...)</tt> \throw GPhoto2pp::exceptions::gphoto2_exception
     */
    gphoto2::_CameraWidget *getChildByNameWrapper(std::string const &name) const;

    /**
     * \brief Gets the child widget by the label passed in and returns the widget type that the user
     * expects. \tparam a type that inherits from CameraWidgetWrapper \param[in]	label	of
     * the child widget to get \return the widget \note Direct wrapper for
     * <tt>gp_widget_get_child_by_label(...)</tt> \throw GPhoto2pp::exceptions::gphoto2_exception
     */
    gphoto2::_CameraWidget *getChildByLabelWrapper(std::string const &label) const;

    /**
     * \brief Gets the child widget at the specified index and returns the widget type that the user
     * expects. \tparam a type that inherits from CameraWidgetWrapper \param[in]	index	of
     * the child widget to get \return the widget \note Direct wrapper for
     * <tt>gp_widget_get_child(...)</tt> \throw GPhoto2pp::exceptions::IndexOutOfRange \throw
