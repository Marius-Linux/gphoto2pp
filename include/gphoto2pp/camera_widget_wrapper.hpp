 ...
 * This class provides a RAII wrapper around the gphoto2 CameraWidget struct. In the gphoto2 the
 * camera's abilities are represented by an N-ary tree.
 *
 * Gphoto2 has 9 node types. 7 of them compose the leaf nodes of the tree and contian read, or
 * read/write values. 2 of them (window and section) are non value widgets, and represent the
 * internal nodes with 1 or more children.
 *
 * Every camera abilities tree has Only 1 root node, which is always of type *Window*. Then this can
 * have n children, typically they are *Section* widgets.
 *
 * Please review the above graphical tree to look at the inheritance hierarchy of the widget types
 * to understand the underlying value they each represent.
 *
...
public:
     ...
     * RAII indicates we still should allow our users access to the RAW Resource and it is
     * applicable. \return the gphoto2 CameraWidget struct
     */
     ...
     * This is the immediate parent of the widget. If the current widget is a leaf node, then this
     * is likely a Section Widget, but could also be a Window Widget. \return the parent widget
     * \note Direct wrapper for gp_widget_get_parent(...) \throw
     * GPhoto2pp::exceptions::gphoto2_exception
     */
    CameraWidgetWrapper getParent() const;
    /**
     * \brief Gets the widget's read-only status.
     * \return the parent read-only status
     * \note Direct wrapper for gp_widget_get_readonly(...)
     * \throw GPhoto2pp::exceptions::gphoto2_exception
     */
