 ...
 * A templated class which must be inherited by any leaf widget type. That means any widget type
 * that is not a Section or a Widget. That way their specific widget implementations can define
 * meaningful Get and Set methods. \tparam the meaningful value type for the widget
 */
template <typename T>
...
protected:
    ValueWidgetBase(gphoto2::_CameraWidget *cameraWidget) : CameraWidgetWrapper{cameraWidget} {
        // Empty Constructor
    }
};
