/** \file
 * \brief A flexible implementation of the observer pattern with automatic lifetime management. This
 * implementation was taken mostly (with some modifications) from
 * https://github.com/WouterLammers/observer \author Copyright (c) 2013 WouterLammers
 * <https://github.com/WouterLammers>, Copyright (c) 2013 maldworth <https://github.com/maldworth>
 *
 * \note
...
protected:
    ...
    Registration registerObserver(const EventTypeBase &e, UniversalPtr fptr) {
        ...
        return createEmptyPtr([e, fptr, weakHeartBeat, this](void *) {
            if (auto isBeating = weakHeartBeat.lock()) {
                observers_[e].erase(std::remove(begin(observers_[e]), end(observers_[e]), fptr),
                                    end(observers_[e]));
            }
        });
    }
