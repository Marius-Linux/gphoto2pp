        ...
        if (tempStream >> pictureQty) {
            if (pictureQty < 0 || pictureQty > 100) {
                ...
                for (int i = 0; i < pictureQty; i++) {
                    gphoto2pp::helper::capturePreview(
                        cameraWrapper, "example7_preview" + std::to_string(i) + ".jpg");
                }
                std::cout << "Done! check the directory which example7 executed in, the pictures "
                             "were saved there"
                          << std::endl;
            }
        } else {
