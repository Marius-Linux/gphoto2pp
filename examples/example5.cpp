...
// I Recommend you go read my synopsis on widgets first, found here:
// http://maldworth.github.io/gphoto2pp/classgphoto2pp_1_1_camera_widget_wrapper.html#details

int main(int argc, char *argv[]) {
    try {
        // I've made this example more complicated than it needs to be, but the purpose is to show
        // that there are many types of widgets that control the cameras, but for simplicity sake,
        // there are really just two methods being used here:
        // 1. getAllWidgetsNames(...)
        // 2. getAllWidgetsOfType(...)

        gphoto2pp::CameraWrapper cameraWrapper{}; // Not passing in model and port will connect to
        ...
        auto getWidgetsOfType = [&](gphoto2pp::CameraWidgetTypeWrapper widgetType) {
            auto allWidgetsOfType =
                gphoto2pp::helper::getAllWidgetsNamesOfType(rootWidget, widgetType, showFullName);
        ...
        do {
            ...
            std::cout << "10. Toggle show full widget name (currently = " << std::boolalpha
                      << showFullName << ")" << std::endl;
            ...
            if (tempStream >> menuNumber) {
                ...
                {
                    auto allWidgets =
                        gphoto2pp::helper::getAllWidgetsNames(rootWidget, showFullName);
                ...
                case 10: // Toggle showing full name
                {
                    ...
                    std::cout << std::endl
                              << "Full widget name " << (showFullName ? "WILL" : "will NOT")
                              << " be visible" << std::endl
                              << std::endl;
                    break;
                }
