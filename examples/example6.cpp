...
void doDateWidget(gphoto2pp::CameraWrapper &cameraWrapper, bool readOnly) {
    ...
    auto dateWidget = rootWidget.getChildByName<gphoto2pp::DateWidget>(
        "datetime"); // we know the widget name is 'datetime' by executing example5 and finding out
                     // the camera's available date widgets
    ...
    std::cout << "\t   type:\t" << static_cast<int>(dateWidget.getType())
              << std::endl; // c++11 strongly typed enums used
...
void doToggleWidget(gphoto2pp::CameraWrapper &cameraWrapper, bool readOnly) {
    ...
    if (readOnly == false) {
        // Toggling this to 1 makes the lens perform the autofocus. Not sure what happens for mf
        // lens. probably nothing...
...
void doButtonWidget(gphoto2pp::CameraWrapper &cameraWrapper, bool readOnly) {
    std::cout
        << std::endl
        << "Nikon D90 unfortunately does not have any button widgets, but you are free to put "
           "your own test implementation here!"
        << std::endl;
}
...
void doRangeWidget(gphoto2pp::CameraWrapper &cameraWrapper, bool readOnly) {
    ...
    // Other range ones: autowhitebias, manualfocusdrive, burstnumber, flashexposurecompensation,
    // lightmeter, cloudywhitebias, flashwhitebias, lowlight, shadewhitebias, focallength,
    // flexibleprogram
    auto rangeWidget =
        rootWidget.getChildByName<gphoto2pp::RangeWidget>("flashexposurecompensation");
...
void doMenuWidget(gphoto2pp::CameraWrapper &cameraWrapper, bool readOnly) {
    ...
    // 4=RAW, 5=RAW+basic, 6=RAW+normal, 7=RAW+fine 5003 Corresponds to Nikon D90 Image size menu
    // (only relevant for 5004 options with Jpeg in it)
...
void doRadioWidget(gphoto2pp::CameraWrapper &cameraWrapper, bool readOnly) {
    ...
    // Now we will get the iso widget. We could use the name 'iso', but to change it up, we will use
    // the label instead
...
void doSectionWidget(gphoto2pp::CameraWrapper &cameraWrapper) {
    ...
    // We could loop through all the children, from 0 to countChildren(), and then determine each
    // child type and create the widget, but that's an exercise you can complete yourself if you so
    // wish.
}
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        // For my Nikon D90, I will be getting and setting one of each type of widget (where
        // possible)
        ...
        // I'm using a nikon D90, if your camera is of another brand, some of the commands below
        // might behave differently, and change different settings on your camera.
        ...
        do {
            ...
            std::cout << "#############################" << std::endl;
            std::cout
                << "Please note that some of these tests might change settings on your camera "
                   "(iso, date, etc...). Please read through the example6.cpp for details"
                << std::endl;
            ...
            std::cout << "9. Toggle Read vs ReadWrite (currently = "
                      << (readOnly ? "read only" : "read write") << ")" << std::endl;
            ...
            if (tempStream >> menuNumber) {
                ...
                case 9: // Toggle showing full name
                {
                    readOnly = !readOnly;
                    std::cout << std::endl
                              << "Widget tests will now " << (readOnly ? "read only" : "read write")
                              << std::endl
                              << std::endl;
                    break;
                }
