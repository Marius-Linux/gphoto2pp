...
// This method is only needed for certain canon cameras. I had tested this on a Canon EOS Rebel T2i
// (shows as Canon EOS 550D) with the boolean set to true and false, and I noticed no difference. So
// you might need it or you might not depending on what camera you use.
void setCanonCapture(gphoto2pp::CameraWrapper &cameraWrapper, bool capture) {
    try {
        auto captureWidget =
            cameraWrapper.getConfig().getChildByName<gphoto2pp::ToggleWidget>("capture");
        captureWidget.setValue(capture ? 1 : 0);
        cameraWrapper.setConfig(captureWidget);
    } catch (const std::runtime_error &e) {
        // Swallow the exception
        std::cout << "Tried to set canon capture, failed. The camera is probably not a canon"
                  << std::endl;
    }
}
...
int main(int argc, char *argv[]) {
    try {
        ...
        std::cout << "#############################" << std::endl;
        gphoto2pp::CameraWrapper cameraWrapper; // Not passing in model and port will connect to the
        ...
        // Clean and quick capture and save to disk, but this assumes you are taking images, and in
        // jpeg foramt. Adjust type and extension as appropriate.
        ...
        std::cout << "FileName (empty by default): " << cameraFileWrapper.getFileName()
                  << std::endl;

        std::cout << "MimeType: " << cameraFileWrapper.getMimeType() << std::endl;

        // This will only set the extension for known mime types (application/unknown is not
        // understood, and you should perform detectMimeType before this method)
        cameraFileWrapper.adjustNameForMimeType();
        std::cout << "FileName (still empty if the mime type is still unknown): "
                  << cameraFileWrapper.getFileName() << std::endl;
        ...
        // Again, the release notes say that the setFileName and getFileName will be going away,
        // which I understand, because it's not until we call the .save(fileName) method, that it's
        // saved to the place with the directory.
        cameraFileWrapper.setFileName("TestRandom0293.odf");

        std::cout << "Delibrately incorrect filename and extension: "
                  << cameraFileWrapper.getFileName() << std::endl;

        // Should Fix the extension
        cameraFileWrapper.adjustNameForMimeType();

        std::cout << "FileName (proper extension): " << cameraFileWrapper.getFileName()
                  << std::endl;

        // Now after all that fiddling with filenames, we save the file and use our own filename
        // (haha)
        std::cout << "FileName we will save with: " << fileName << std::endl;
        cameraFileWrapper.save(fileName);

        // Now lets take another picture but ignore all of that messing around we did above and just
        // use 4 lines of code instead :)
        ...
        std::fstream f1("example4_capture_method_3.jpg",
