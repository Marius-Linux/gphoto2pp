...
int main(int argc, char *argv[]) {
    try {
        ...
        auto mylogger = [](gphoto2pp::helper::debugging::LogLevelWrapper level,
                           const std::string &domain, const std::string &str, void *data) {
            auto now = std::chrono::high_resolution_clock::now();
            auto us = std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch());
            auto s = std::chrono::duration_cast<std::chrono::seconds>(us);
            std::cout << (s.count() % 60) << "." << (us.count() % 1000000) << ": ";

            // This line is really the only important one, the chrono stuff is so it mimics the
            // gphoto2 debugger output
            std::cout << str << std::endl;

            // If you want to log it using the same mechanism as gphoto2pp, use this line instead of
            // cout FILE_LOG(logDEBUG) << (s.count() % 60) << "." << (us.count() % 1000000) << ": "
            // << str;
        };

        // This registration variable should be kept in scope for as long as you want it to log.
        // Once the registration variable falls out of scope (and is cleaned up), the handler loses
        // registration.
        auto registration = gphoto2pp::helper::debugging::subscribeToPortLogEvents(
            gphoto2pp::helper::debugging::LogLevelWrapper::Debug, mylogger);

        // Don't start actual listening until this is triggered
        gphoto2pp::helper::debugging::startPortLogging(
            gphoto2pp::helper::debugging::LogLevelWrapper::Debug);
