...
gphoto2pp::ToggleWidget getViewfinder(gphoto2pp::WindowWidget &rootWidget) {
    try {
        ...
        std::cout << "Tried to get 'viewfinder' couldn't find, will try to get 'eosviewfinder'"
                  << std::endl;
...
void resetManualFocus(gphoto2pp::CameraWrapper &cameraWrapper, int focusTo) {
    ...
    try {
        ...
        // TODO, need to add the focus in for canon. I don't have a canon camera to test the
        // settings, but I will try to borrow from a friend
    }

    // Save the focus changes to the camera.
    cameraWrapper.setConfig(rootWidget);

    // Pause for half a second to let the focus motor do it's thing. This is probably way longer
    // than necessary.
...
int main(int argc, char *argv[]) {
    try {
        ...
        // First, we have to enable the viewfinder, which will enter the camera into viewfinder
        // mode.
        viewFinder.setValue(1);
        cameraWrapper.setConfig(viewFinder);

        // *NOTE* If the camera wasn't in viewfinder mode, then we would not be allowed to write the
        // manualfocusdrive 		  So it becomes apparent that some widgets depend on the
        // camera being in certain states, and so you will have become familiar with tinkering
        // camera settings to get things to behave as you expect
        ...
        // So 10 iterations with the focus of 2 (translates to 512), will be about 5000, and should
        // cover the whole lens (from close to infinity) I just used the arbitrary focus values from
        // the example in libgphoto2 (focus.c), you are free to set your own values that are smaller
        // or larger than 256 and 1024 respectively

        // I set it at minimum, this should be lens independent and just setting the focus as close
        // as possible (at least on my lens it sets it to as close as possible) Then I will only
        // need to use positive increments in the forloop to adjust the focus in the positive
        // direction (manualfocus set value is relative) You can set negative values to go backwards
        // as well. But if you wanted to do that, I'd suggest then using the min (-1) here, and
        // going in reverse to -5000 instead.
        resetManualFocus(cameraWrapper, -1);

        // The first picture on my Nikon D90 always seems to be blank, so we will take a dummy
        // picture
        gphoto2pp::helper::capturePreview(cameraWrapper, "example8_dummy.jpg");

        // This loop is slower, but it shows you how the value is actually relative, and each time
        // the getValue is zero.
        for (int i = 0; i < loopqty; i++) {
            // this next line is unnecessary and slows everything down, but I do it just to show
            // that the current focus value always is '0' (At least on my Nikon D90), thus setting
            // it is a relative increment.
            setManualFocus(cameraWrapper, focusStep);
            gphoto2pp::helper::capturePreview(cameraWrapper,
                                              "example8_preview_slower" + std::to_string(i) +
                                                  "_mfval(" + std::to_string(focusStep) + ").jpg");
        }
