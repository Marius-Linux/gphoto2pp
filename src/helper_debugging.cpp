...
static void errordumper(gphoto2::GPLogLevel level, const char *domain, const char *format,
                        va_list args, void *data) {
    std::string str = strformat(format, args);
#else
static void errordumper(gphoto2::GPLogLevel level, const char *domain, const char *str,
                        void *data) {
#endif

    switch (level) {
    case gphoto2::GP_LOG_ERROR: {
        PortLogEventHandler(LogLevelWrapper::Error, static_cast<LogLevelWrapper>(level),
                            std::string(domain), std::string(str), data);
    }
    case gphoto2::GP_LOG_VERBOSE: {
        PortLogEventHandler(LogLevelWrapper::Verbose, static_cast<LogLevelWrapper>(level),
                            std::string(domain), std::string(str), data);
    }
    case gphoto2::GP_LOG_DEBUG: {
        PortLogEventHandler(LogLevelWrapper::Debug, static_cast<LogLevelWrapper>(level),
                            std::string(domain), std::string(str), data);
    }
    case gphoto2::GP_LOG_DATA: {
        PortLogEventHandler(LogLevelWrapper::Data, static_cast<LogLevelWrapper>(level),
                            std::string(domain), std::string(str), data);
    }
    default: {
        break;
...
PortLoggingEventsManager::~PortLoggingEventsManager() { stopPortLogging(); }

void PortLoggingEventsManager::startPortLogging(LogLevelWrapper const &level) {
    if (id == 0) {
        id = checkResponse(
            gphoto2::gp_log_add_func(static_cast<gphoto2::GPLogLevel>(level), errordumper, nullptr),
            "gp_log_add_func");
    }
}
...
void stopPortLogging() { detail::PortLogEventHandler.stopPortLogging(); }

observer::Registration
subscribeToPortLogEvents(LogLevelWrapper const &event,
                         std::function<void(LogLevelWrapper const &level, std::string const &domain,
                                            std::string const &str, void *data)>
                             func) {
