...
ChoicesWidget::ChoicesWidget(gphoto2::_CameraWidget *cameraWidget) : StringWidget{cameraWidget} {
    switch (this->getType()) {
...
int ChoicesWidget::countChoices() const {
    return gphoto2pp::checkResponse(gphoto2::gp_widget_count_choices(m_cameraWidget),
...
int ChoicesWidget::getChoice() const {
    ...
    if (item == std::end(choices)) {
        throw exceptions::ValueOutOfLimits("For some strange reason, the current value set on the "
                                           "camera didn't match to a value from the choices");
    }
...
void ChoicesWidget::setChoice(int index) {
    if (index >= countChoices()) {
        throw exceptions::IndexOutOfRange(
            "You are trying to set a choice index which is greater than the maximum choice index.");
    }
...
std::string ChoicesWidget::choiceToString(int index) const {
    if (index >= countChoices()) {
        throw exceptions::IndexOutOfRange(
            "You are trying to get a choice index which is greater than the maximum choice index.");
    }

    char const *temp = nullptr;

    gphoto2pp::checkResponse(gphoto2::gp_widget_get_choice(m_cameraWidget, index, &temp),
...
std::string ChoicesWidget::choicesToString(std::string &&separator /* = " " */) const {
    ...
    return temp.str().substr(0,
                             temp.str().length() - separator.length()); // erases the last separator
}
