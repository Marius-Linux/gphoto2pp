...
void capturePreview(CameraWrapper &cameraWrapper, std::string const &outputFilename) {
    CameraFileWrapper cameraFile =
        cameraWrapper.capturePreview(); // No point in duplicating code, call overloaded method
...
void capturePreview(CameraWrapper &cameraWrapper, std::ostream &outputStream) {
    ...
    outputStream.flush();
    // Not sure if I should close the file in here, or let the user do it. I'll let the user do it
    // for now.
}

void capture(CameraWrapper &cameraWrapper, CameraFileWrapper &cameraFile,
             bool autoDeleteImageFromSrc /* = false */,
...
void capture(CameraWrapper &cameraWrapper, std::ostream &outputStream,
    ...
    outputStream
        .flush(); // If we don't flush, I found strange things might happen to the jpg, for one
                  // thing the thumbnail wouldn't show up. Makes sense, as once we leave this scope
                  // some items are disposed, so we want to make sure that the stream is flushed.
    // Not sure if I should close the file in here as well, or let the user do it. I'll let the user
    // do it for now.
...
// Private Method
void getChildrenItems(CameraWrapper &cameraWrapper, std::string const &folder,
                      std::vector<std::string> &allItems, bool getFiles) {
    if (getFiles == false) {
    ...
    for (int i = 0; i < cameraListFolders.count(); ++i) {
        getChildrenItems(cameraWrapper, folder + cameraListFolders.getName(i) + "/", allItems,
                         getFiles);
    }
}
