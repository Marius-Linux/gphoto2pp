...
namespace gphoto2pp {
CameraFileWrapper::CameraFileWrapper() : m_cameraFile(nullptr) {
...
CameraFileWrapper::CameraFileWrapper(CameraFileWrapper &&other) : m_cameraFile(other.m_cameraFile) {
...
gphoto2::_CameraFile *CameraFileWrapper::getPtr() const { return m_cameraFile; }
...
std::vector<char> CameraFileWrapper::getDataAndSize() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_file_get_data_and_size(m_cameraFile, &buffer, &size),
    ...
    try {
        if (file.empty() == false) {
            ...
            gphoto2pp::checkResponse(
                gphoto2::gp_file_set_data_and_size(m_cameraFile, myCopy, file.size()),
                "gp_file_set_data_and_size");
            // We don't delete myCopy because ownership was transferred to the m_cameraFile struct
            // along with the contents
        }
    } catch (...) {
        // Just in case, we catch any exception and delete our myCopy, because we can assume it
        // failed and we don't want that memory leaking
        delete[] myCopy;
        throw;
    }
}
...
std::string CameraFileWrapper::getMimeType() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_file_get_mime_type(m_cameraFile, &temp),
...
void CameraFileWrapper::setMimeType(std::string const &mimeType) {
    gphoto2pp::checkResponse(gphoto2::gp_file_set_mime_type(m_cameraFile, mimeType.c_str()),
...
void CameraFileWrapper::setFileName(std::string const &fileName) {
    gphoto2pp::checkResponse(gphoto2::gp_file_set_name(m_cameraFile, fileName.c_str()),
                             "gp_file_set_name");
}

void CameraFileWrapper::detectMimeType() const {
    gphoto2pp::checkResponse(gphoto2::gp_file_detect_mime_type(m_cameraFile),
                             "gp_file_detect_mime_type");
}

void CameraFileWrapper::adjustNameForMimeType() {
    gphoto2pp::checkResponse(gphoto2::gp_file_adjust_name_for_mime_type(m_cameraFile),
                             "gp_file_adjust_name_for_mime_type");
}

std::string CameraFileWrapper::getNameByType(std::string const &baseName,
                                             CameraFileTypeWrapper const &type) const {
#ifdef GPHOTO_LESS_25
    ...
    gphoto2pp::checkResponse(
        gphoto2::gp_file_get_name_by_type(m_cameraFile, baseName.c_str(),
                                          static_cast<gphoto2::CameraFileType>(type), &temp),
...
void CameraFileWrapper::save(std::string const &filename) const {
    // Not sure if we should expose this. in gphoto2-file.h it mentions this method was never meant
    // for public api usage, but it is handy and lots of people use it... I will add it now for
    // completness
