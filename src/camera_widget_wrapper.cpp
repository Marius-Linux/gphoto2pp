...
CameraWidgetWrapper::CameraWidgetWrapper(gphoto2::_CameraWidget *cameraWidget)
    ...
    // This call to increment the reference is correct 99% of the time when this constructor is
    // called because it is likely being called for a child widget. However the one scenario when we
    // call this constructor from the CameraWrapper.getConfig, it is the parent, and so we have now
    // just incremented the reference to == 2. This is wrong, and so we unref right after in
    // CameraWrapper::getConfig
    ref();
}
...
CameraWidgetWrapper::CameraWidgetWrapper(CameraWidgetWrapper &&other)
    ...
    other.m_cameraWidget =
        nullptr; // So when the 'other' instance calls it's destructor, it won't try to unreference.
}
...
gphoto2::_CameraWidget *CameraWidgetWrapper::getPtr() const { return m_cameraWidget; }

std::string CameraWidgetWrapper::getName() const {
    const char *temp = nullptr;

    gphoto2pp::checkResponse(gphoto2::gp_widget_get_name(m_cameraWidget, &temp),
                             "gp_widget_get_name");
...
CameraWidgetTypeWrapper CameraWidgetWrapper::getType() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_widget_get_type(m_cameraWidget, &temp),
                             "gp_widget_get_type");
...
std::string CameraWidgetWrapper::getLabel() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_widget_get_label(m_cameraWidget, &temp),
                             "gp_widget_get_label");
...
std::string CameraWidgetWrapper::getInfo() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_widget_get_info(m_cameraWidget, &temp),
                             "gp_widget_get_info");
...
bool CameraWidgetWrapper::getReadOnly() const {
    // Because of C doesn't have bool
    int readonly = false;

    gphoto2pp::checkResponse(gphoto2::gp_widget_get_readonly(m_cameraWidget, &readonly),
...
gphoto2::_CameraWidget *CameraWidgetWrapper::getRootDefault() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_widget_get_root(m_cameraWidget, &rootWidget),
                             "gp_widget_get_root");
...
gphoto2::_CameraWidget *CameraWidgetWrapper::getParentDefault() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_widget_get_parent(m_cameraWidget, &parentWidget),
