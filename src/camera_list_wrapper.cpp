...
CameraListWrapper::CameraListWrapper() : m_cameraList{nullptr} {
    gphoto2pp::checkResponse(gphoto2::gp_list_new(&m_cameraList), "gp_list_new");
}

CameraListWrapper::~CameraListWrapper() {
    if (m_cameraList != nullptr) // Added this line so the move constructor wouldn't dispose of the
                                 // object if it was reassigned
    {
...
CameraListWrapper::CameraListWrapper(CameraListWrapper &&other) : m_cameraList{other.m_cameraList} {
...
CameraListWrapper::CameraListWrapper(CameraListWrapper const &other)
    : m_cameraList{other.m_cameraList} {
    // Because we now refer to the same cameralist as "other", we need to add to it's reference
    // count
    if (m_cameraList != nullptr) {
...
gphoto2::_CameraList *CameraListWrapper::getPtr() const { return m_cameraList; }
...
std::string CameraListWrapper::getName(int index) const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_list_get_name(m_cameraList, index, &temp),
                             "gp_list_get_name");
...
std::string CameraListWrapper::getValue(int index) const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_list_get_value(m_cameraList, index, &temp),
                             "gp_list_get_value");
...
void CameraListWrapper::append(std::string const &name, std::string const &value) {
    gphoto2pp::checkResponse(gphoto2::gp_list_append(m_cameraList, name.c_str(), value.c_str()),
                             "gp_list_append");
}
...
int CameraListWrapper::findByName(std::string const &name) const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_list_find_by_name(m_cameraList, &index, name.c_str()),
...
void CameraListWrapper::setName(int index, std::string const &name) {
    gphoto2pp::checkResponse(gphoto2::gp_list_set_name(m_cameraList, index, name.c_str()),
                             "gp_list_set_name");
}

void CameraListWrapper::setValue(int index, std::string const &value) {
    gphoto2pp::checkResponse(gphoto2::gp_list_set_value(m_cameraList, index, value.c_str()),
                             "gp_list_set_value");
}
...
CameraListWrapper::getPairByName(std::string const &name) const {
