...
CameraListWrapper autoDetectAll() {
    ...
    gphoto2pp::checkResponse(
        gphoto2::gp_camera_autodetect(cameraListWrapper.getPtr(), spContext.get()),
...
int checkResponse(int result, std::string &&methodName) {
    if (result < 0) {
        ...
        errorMessage << methodName << ": failed with return code '" << result
                     << "' and the reason is: '" << gphoto2::gp_result_as_string(result) << "'";
