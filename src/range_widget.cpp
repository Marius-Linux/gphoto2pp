...
RangeWidget::RangeWidget(gphoto2::_CameraWidget *cameraWidget) : FloatWidget{cameraWidget} {
...
RangeWidgetRange RangeWidget::getRange() const {
    ...
    gphoto2pp::checkResponse(
        gphoto2::gp_widget_get_range(m_cameraWidget, &range.Min, &range.Max, &range.Step),
        "gp_widget_get_range");
    return range;
}
...
std::string RangeWidget::ToString() const {
    ...
    gphoto2pp::checkResponse(gphoto2::gp_widget_get_range(m_cameraWidget, &m_min, &m_max, &m_step),
                             "gp_widget_get_range");
